...
public class EntityService {
            ...
            if (redisCandles.containsKey(key)) {
                reportRecords.add(
                        buildRecord(
                                ...
                                redisCandles.get(key).size() > 0 ? getProperRedisEntity(redisCandles.get(key), niftyEntry.getValue().get(0)) : null
                        ));
            }
        }
        ...
        for (Map.Entry<String, List<Entity>> redisEntry : redisCandles.entrySet()
            ...
            if (!niftyCandles.containsKey(key)) {
                ...
                reportRecords.add(
                        buildRecord(
                                null,
                                redisCandles.get(key).size() > 0 ? getProperRedisEntity(redisCandles.get(key), ((List<Entity>)(niftyCandles.values().toArray()[0])).get(0)) : null
                        ));
            }
        }
    ...
    private Entity getProperRedisEntity(List<Entity> list, Entity niftyEntity) {
        return list.stream()
                .filter((Entity entity) -> entity.getTime() == (niftyEntity.getTime()))
    ...
    private ReportRecord buildRecord(Entity entity1, Entity entity2) {
        if (entity1 != null && entity2 == null) {
            return ReportRecord.builder()
                    .time(LocalDateTime.now(ZoneId.of(appProps.getProperty("upstox.compare.time-zone"))))
    ...
    private Candle generateCandle(Entity entity1, Entity entity2) {
        if (entity1 != null && entity2 != null)
            return Candle.builder()
                    .time(entity1.getTime() - entity2.getTime())
        ...
        if (entity1 == null && entity2 != null)
            return Candle.builder()
                ...
                .time(entity1 != null ? entity1.getTime() : 0)
