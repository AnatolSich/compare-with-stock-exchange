...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            JsonUtils jsonService = new JsonUtils(appProps, matchService, dateTimeService);
            ...
            jsonService.getJsonFromFile();

            Map<String, List<Entity>> mapRedisEntities = getRedisData(entityService, redisClient);
            Map<String, List<Entity>> mapNiftyEntities = getNiftyData(jsonService);

            List<ReportRecord> report = entityService.getReportRecords(mapNiftyEntities, mapRedisEntities);
            String key = appProps.getProperty("aws.s3.report.key") + " " + dateTimeService.getFormattedNiftyCurrentLocalDateTime();
            if (report.isEmpty()) {
                log.info(key + " is NOT uploaded to AWS");
            } else {
                cloudStorageClient.uploadString(key, objectMapper.writeValueAsString(report));
            }
    ...
    private static CloudStorageClient loadAmazonClient(Properties appProps, DateTimeService dateTimeService) {
        AmazonS3 s3client = AmazonS3ClientBuilder
                ...
                .build();
        return new CloudStorageClient(appProps.getProperty("aws.s3.bucket.name"), s3client);
    }
    ...
    private static Map<String, List<Entity>> getNiftyData(JsonUtils jsonService) {
        return jsonService.parseFromNiftyToMap(jsonService.parseStringToJson());
    }
